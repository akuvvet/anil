...
');}
  ...
  function compareFlowRows(a,b){ const td=String(a.dataset.tarih||'').localeCompare(String(b.dataset.tarih||'')); if(td!==0)return td; const sb=padSaat(a.dataset.saatBaslangic).localeCompare(padSaat(b.dataset.saatBaslangic)); if(sb!==0)return sb; return padSaat(a.dataset.saatBitis).localeCompare(padSaat(b.dataset.saatBitis)); }
  function sortFlowTable(){ if(editingRow)return; const tbody=document.querySelector('#tableWrap tbody'); if(!tbody)return; const rows=Array.from(tbody.querySelectorAll('tr.flow-row')); rows.sort(compareFlowRows); for(const r of rows)tbody.appendChild(r); }
